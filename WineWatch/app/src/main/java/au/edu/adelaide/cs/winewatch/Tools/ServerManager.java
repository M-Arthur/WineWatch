...
public class ServerManager {
	...
	private static final String CHANGE_INTERVAL = "heartbeat/update_command";
	...
	private static final String REGISTER_ID = "gcm_key";
...
//	}
	...
	public Map<String, Object> login(User user) {
		...
		try {
			...
			Log.e("sever_login", user.getRegid());
			params.add(new BasicNameValuePair(USERNAME, user.getUsername()));
			params.add(new BasicNameValuePair(PASSWORD, user.getPassword()));
			params.add(new BasicNameValuePair(REGISTER_ID, user.getRegid()));
...
//			String content = buf.toString();
			...
			if (response.getStatusLine().getStatusCode() == 200) {
				...
				if (jsonObject.getString(RESULT).equals(LOGIN_SUCCESS)) {
					...
					for (int i = 0; i < ferments.length(); i++) {
						...
						temp.setBaseId(Integer.parseInt(ferments.getJSONObject(i).getString("bid")));
				...
				if (TextUtils.isEmpty(reading.toString())) {
					...
					for (int i = 0; i < reading.length(); i++) {
						...
						try {
							...
							result.clear();
							result.put(Constants.RESULT, false);
							result.put(Constants.INFO,"Server data ");
						}
	...
	public Map<String,Object> changeInterval(User user,int bid, int mid, String command, String parameter){
		String urlPath = SERVER_ADDRESS+CHANGE_INTERVAL;
		List<NameValuePair> params = new ArrayList<NameValuePair>();
