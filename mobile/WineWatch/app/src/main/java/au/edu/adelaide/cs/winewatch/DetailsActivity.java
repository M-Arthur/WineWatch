...
public class DetailsActivity extends ActionBarActivity {
	...
	private static TextView intervalTV;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		intervalTV = (TextView) findViewById(R.id.detail_interval);
	...
	private void initDummyParams() {
		...
		int index = intent.getIntExtra("index_of_ferment", -1);
		if (index == -1) {
			index = 10;
		}
		Ferment ferment = DummyData.getFerment(index);
	...
	private void initPreviousParams() {
		...
		wineryNameTV.setText(winery.getName());
		fermentationTV.setText(fid + "");
		tankIdTV.setText(intent.getStringExtra(Constants.PACKAGE_NAME + "tank_id"));
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		switch (item.getItemId()) {
			case R.id.action_more:
				...
				Intent intent = new Intent();
				intent.setClass(this, TankMoreInfoActivity.class);
		...
		public void run() {
			...
			if (TESTING) {
				...
				if ((boolean) result.get(Constants.RESULT)) {
					...
					temperatures = temp.getTemperatures();
					bundle.putString("update_time", temp.getUpdatingTime());
					bundle.putDouble("battery", temp.getBattery());
					bundle.putDouble("interval", temp.getInterval());
				} else {
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.arg1) {
				case SUCCESS:
					...
					if(interval>1) {
						intervalTV.setText(interval + " hrs");
					}
					else{
						intervalTV.setText(interval+" hr");
					}
